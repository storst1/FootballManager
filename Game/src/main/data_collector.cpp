...
void MainWindow::FillAllLeagues()
{
    ...
    for(int i = 0; i < allLeagues.size(); ++i){
        QVector<API_CLUB*> curCompClubsList = InitClubs(netManager->GatherClubsListByComp(allLeagues[i]->getId()));
...
{
    QVector<QPair<QString, int>> allLeaguesInfo = realDataDb->getAllLeagues();
...
void MainWindow::FillAllPlayers()
{
    for(int i = 0; i < allClubs.size(); ++i){
        QVector<API_PLAYER*> curSquad = netManager->GatherPlayersListByClub(allClubs[i]->getId());
...
QVector<API_CLUB *> MainWindow::InitClubs(QVector<API_CLUB> clubList)
{
    QVector<API_CLUB*> clubsList;
